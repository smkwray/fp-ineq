...
function getDictionaryRecord(variable) {
  const record = state.dictionary.get(variable);
  if (record) {
    return {
      code: variable,
      ...record,
...
function parseEquationFormulaMetadata(formula) {
  const text = `${formula || ""}`;
  const lhsVariables = [];
  const rhsVariables = new Set();
  const functionNames = new Set(["EXP", "LOG", "MIN", "MAX", "ABS"]);
  ...
  for (const statement of statements) {
    ...
    const matches = rhs.toUpperCase().match(/[A-Z][A-Z0-9_]*/g) || [];
    for (const token of matches) {
      if (functionNames.has(token) || lhsVariables.includes(token)) {
        continue;
      }
      rhsVariables.add(token);
    }
  }

  return {
    lhs_variables: lhsVariables,
    lhs_expr: lhsVariables.length > 0 ? lhsVariables.join(", ") : "",
    rhs_variables: [...rhsVariables],
  };
}

function normalizeEquationRecord(id, value) {
  const record = value && typeof value === "object" ? value : {};
  const parsed = parseEquationFormulaMetadata(record.formula);
  return {
    id: `${id}`,
    display_id: /^\d+$/.test(`${id}`) ? `Eq ${id}` : `${id}`,
    type: record.type || "identity",
    label: record.label || `Equation ${id}`,
    formula: record.formula || "",
    lhs_expr: record.lhs_expr || parsed.lhs_expr,
    lhs_variables: Array.isArray(record.lhs_variables) ? record.lhs_variables : parsed.lhs_variables,
    rhs_variables: Array.isArray(record.rhs_variables) ? record.rhs_variables : parsed.rhs_variables,
    source_runs: Array.isArray(record.source_runs) ? record.source_runs : [],
  };
}
...
function collectEquationRefsForVariable(variable) {
  ...
  for (const equation of state.equationCatalog.values()) {
    const lhsValues = Array.isArray(equation.lhs_variables) && equation.lhs_variables.length > 0
      ? equation.lhs_variables.map((name) => `${name}`.trim().toUpperCase())
      : [`${equation.lhs_expr || ""}`.trim().toUpperCase()].filter(Boolean);
    ...
    if (lhsValues.includes(variable)) {
...
function renderVariableEquationLookup(variable) {
  ...
  const sourceRuns = formatSourceRuns(record);
  if (sourceRuns) {
    summary.appendChild(createTextElement("p", sourceRuns, "equation-note"));
  }
...
async function initialize() {
  ...
  state.equationCatalog = new Map(
    Object.entries(dictionaryPayload.equations || {}).map(([key, value]) => [
      `${key}`,
      normalizeEquationRecord(key, value),
    ]),
  );
