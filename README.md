...
The current bridge is therefore a public comparison scaffold, not a settled cross-repo validation result. `ea-ineq` now publishes a polarity audit that rules out a simple sign-convention flip, and its side-by-side compare now uses full `fp-ineq` channel envelopes rather than temporary anchor scenarios. Public interpretation should still stay limited because the deeper estimand mismatch remains unresolved.
